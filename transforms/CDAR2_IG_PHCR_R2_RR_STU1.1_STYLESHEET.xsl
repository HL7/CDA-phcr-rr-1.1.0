...
-->
    ...
    <!-- show-contactInfo -->
    <!-- SG: 20211020 Added processing for useable period (for RR only want current address - i.e. if no useablePeriod.high 
             this includes useablePeriod not being there at all - also if all fields in address are null, don't bother with it at all-->
    <xsl:template name="show-contactInfo">
        <xsl:param name="contact" />
        <!--<xsl:variable name="vNotNullAddress"> 
    ...
    <xsl:template name="show-address">
        ...
        <xsl:choose>
            <xsl:when test="$address">
                ...
                <xsl:if test="string-length($address/n1:streetAddressLine) > 0">
